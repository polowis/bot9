...
bot.on("guildBanAdd", (guild) => {
	guild.fetchAuditLogs().then((logs) => {
		...
		const boner = bane[ser.id + guild.id];
		banse.add(ser.id);
		...
		if (boner.bans > 2) {
			const roles = guild.members.get(ser.id).roles.array();
...
bot.on("message", async (message) => {
	...
	if (command == "v2") {
		...
		const c = result.intent;
		console.log(result.intent);
		const name = c.split(".")[0];
		console.log(name);
		if (name == "commands") {
			try {
				const botcommand = c.split(".")[1];
				console.log(botcommand);
				const commandFile = require(`./commands/${botcommand}.js`);
